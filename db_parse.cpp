...
void db_parse(bool print)
{
  ...
  } else if (!prefix) { // My filepath
      const char *homeDirectory = getenv("HOME");
      if (homeDirectory) {
        size_t pathLength = strlen(homeDirectory) + strlen("/Documents/ComS327/pokedex/") + 1;
        prefix = (char *)malloc(pathLength);
        if (prefix) {
          strcpy(prefix, homeDirectory);
          strcat(prefix, "/Documents/ComS327/pokedex/");
        }
      }
    }
